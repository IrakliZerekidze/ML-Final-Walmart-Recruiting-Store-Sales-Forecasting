...
│   └── XGBoost-ის ექსპერიმენტები feature engineering-ით,
...
მონაცემები არ არის ერთ დიდ დროით რიგში, რომელზეც ერთი სეზონური მოდელის მორგება იქნებოდა საკმარისი. რეალურად გვაქვს ათასობით პარალელური დროითი რიგი: მაგალითად, `Store=1, Dept=1` ერთი სერიაა, `Store=1, Dept=2` მეორე, ხოლო იგივე დეპარტამენტი სხვა მაღაზიაში უკვე ცალკე სერიას ქმნის. ამ სერიებს ერთმანეთისგან განსხვავებული მასშტაბი, ისტორიის სიგრძე, სეზონურობა და holiday-ებზე რეაქცია აქვთ.
...
ეს განსხვავება მნიშვნელოვანია, რადგან, მაგალითად, Thanksgiving-ის კვირაში დაშვებული 1,000 ერთეულის შეცდომა საბოლოო score-ზე ხუთჯერ უფრო ძლიერ მოქმედებს, ვიდრე იგივე შეცდომა ჩვეულებრივ კვირაში. შედეგად, მხოლოდ საშუალო პერიოდის კარგად პროგნოზირება საკმარისი არ არის — მოდელმა holiday spikes-იც უნდა დაიჭიროს.

Training-ის დროს ყველა მოდელში პირდაპირ WMAE loss არ გამოგვიყენებია. NeuralForecast მოდელებში loss იყო MAE, LightGBM-სა და XGBoost-ში კი საუკეთესო შედეგები L1 objective-მა მოგვცა. საბოლოო შეფასება ყველა შემთხვევაში ერთნაირი WMAE ფუნქციით შესრულდა, რათა სხვადასხვა ოჯახის მოდელები სამართლიანად შეგვედარებინა.
...
ყველაზე მეტი missing value გვხვდება `MarkDown1–MarkDown5` სვეტებში. ეს მონაცემები მხოლოდ ისტორიის ნაწილისთვის არის ხელმისაწვდომი, ამიტომ raw MarkDown მნიშვნელობების პირდაპირ გამოყენებასთან ერთად შევქმენით შესაბამისი არსებობის ინდიკატორებიც.
...
`Weekly_Sales` ძლიერ **right-skewed** განაწილებას აჩვენებს: მონაცემების უმეტესობა შედარებით მცირეა, თუმცა გვხვდება ძალიან მაღალი გაყიდვების პიკები. ასევე არსებობს მცირე რაოდენობის უარყოფითი მნიშვნელობები, რომლებიც სავარაუდოდ ნივთის დაბრუნებებს, დაბრუნებებს, refunds-ს უკავშირდება.

<img width="800" height="400" alt="image" src="https://github.com/user-attachments/assets/e04dfafe-a53e-4f24-9075-c6f6ff108253" />

### 3. გაყიდვები დროში

საერთო ყოველკვირეულ გაყიდვებში მკაფიოდ ჩანს წლიური სეზონურობა და ნოემბერ–დეკემბრის მაღალი პიკები. ეს ასაბუთებს წლიური lag-ების, სეზონური სტატისტიკური მოდელებისა და trend/seasonality-ზე ორიენტირებული ნეირონული არქიტექტურების გამოყენებას.
...
### 6. დეპარტამენტების განსხვავება

Department-ებს შორის გაყიდვების მასშტაბი მკვეთრად განსხვავდება. ამიტომ `Dept` უბრალო უწყვეტ რიცხვად არ უნდა აღვიქვათ — tree-based მოდელებში იგი კატეგოტიულ/identifier feature-ად დამუშავდა, ხოლო time-series მოდელებში `(Store, Dept)` წყვილმა `unique_id` შექმნა.
...
Store–Department სერიებს განსხვავებული სიგრძე აქვთ და ყველა მათგანი სრულად უწყვეტი არ არის. ეს განსაკუთრებით მნიშვნელოვანი აღმოჩნდა TFT-ისთვის, რომელსაც ყველა training სერიისთვის სრული future dataframe სჭირდება.
...
- გაყიდვებში გვაქვს ძლიერი **წლიური სეზონურობა** და holiday spikes;
- მონაცემები შედგება ათასობით განსხვავებული Store–Department სერიისგან;
- `MarkDown` feature-ები sparse და ხმაურიანია;
- ისტორიული target, ყოველწლიური lags და კალენდარული feature-ები ყველაზე პერსპექტიული სიგნალებია;
...
გაყიდვების მონაცემები `features.csv`-ს უერთდება `Store`, `Date` და `IsHoliday` სვეტებით, ხოლო `stores.csv` ემატება `Store`. შედეგად, თითოეულ Store–Department–Week observation-ს ახლავს როგორც target-ის ისტორია, ასევე მაღაზიისა და გარე გარემოს შესახებ ინფორმაცია.

### რატომ ვყოფთ დატას grid-ის შევსებამდე

საწყის pipeline-ში სრულ weekly grid-ს ჯერ მთელ dataset-ზე ვქმნიდით და მხოლოდ შემდეგ ვყოფდით train/validation ნაწილებად. ამ მიდგომამ მცირე leakage შექმნა, რადგან training მონაცემში შეიძლებოდა ხელოვნურად გამოჩენილიყო Store–Department წყვილი, რომელიც რეალურად მხოლოდ მომავალ validation პერიოდში ჩნდებოდა. მოდელი ამ გზით მომავალი სერიის არსებობის შესახებ ინფორმაციას იღებდა.

საბოლოო leakage-safe pipeline:
...
საბოლოო შედეგები მხოლოდ შესწორებულ pipeline-ზე მიღებული run-ებიდან შევადარეთ.
...
ასევე შეიქმნა ცალკეული holiday flag-ები, ამერიკაში პოპილარული holiday-ების მიხედვით:
...
local validation score-ის ძირითადი დანიშნულება არის:
...
Median საშუალოზე უფრო გამძლეა holiday spikes-ისა და outlier-ების მიმართ. ის გვიჩვენებს, რამდენად მეტ ინფორმაციას სწავლობს მოდელი უბრალო ისტორიულ level-თან შედარებით.

ყველა საბოლოო მოდელმა, გარდა მარტივი ARIMA-ს ზოგიერთი run-ისა, baseline-ს მნიშვნელოვნად აჯობა. საუკეთესო XGBoost-მა WMAE დაახლოებით 44%-ით შეამცირა, რაც ადასტურებს, რომ სეზონურობა, target სტატისტიკა და არასწფივი ურთიერთქმედებები რეალურ დამატებით სიგნალს შეიცავდა.
...
XGBoost-შიც გამოყენებულია global tabular forecasting. საბოლოო ექსპერიმენტებში LightGBM-ის feature-ები გადავიტანეთ XGBoost-ზე: RichEncoder, yearly lags, YoY ratio, calendar extras, L1 objective და ძლიერი regularization.
...
პირველი ხელით მორგებული XGBoost configuration 1409 WMAE-მდე მივიდა. იგივე RichEncoder, `lag_52`, `lag_104`, YoY ratio, calendar features და L1 objective-ების დამატებამ score 1254.85-მდე შეამცირა და პროექტის საუკეთესო ლოკალური შედეგი მოგვცა.

ასევე შევინარჩუნეთ უფრო მკაცრად რეგულირებული run. მისი WMAE 1261.95 იყო — დაახლოებით 7 ერთეულით უარესი — მაგრამ generalization gap 104-დან 55-მდე შემცირდა. ეს აჩვენებს, რომ მხოლოდ საუკეთესო score-ის არჩევა ყოველთვის საკმარისი არ არის; production ან უცნობ test distribution-ზე უფრო კონსერვატიული კონფიფურაცია შეიძლება მეტად უსაფრთხო იყოს.
...
N-BEATS არის MLP ბლოკებზე დაფუძნებული deep forecasting არქიტექტურა. თითოეული block ქმნის:

- `backcast`-ს — გადასცემს ინფუთის ისტორიას;
- `forecast`-ს — გადასცემს მომავალი პერიოდის პროგნოზს.
...
N-BEATS-ის პირველი მიზანი იყო შეგვემოწმებინა, რამდენად ძლიერი შეიძლება იყოს pure univariate forecasting — ანუ მხოლოდ გაყიდვების ისტორიაზე დაფუძნებული მოდელი, ყოველგვარი Temperature, CPI, MarkDown ან Store Size feature-ის გარეშე. მიუხედავად feature-ების სიმცირისა, მოდელი tree-based შედეგებთან ძალიან ახლოს მივიდა.

საწყის არქიტექტურაში Identity, Trend და Seasonality stacks გამოიყენებოდა. Training steps-ის გაზრდამ და learning rate-ის შემცირებამ ოპტიმიზაცია უფრო სტაბილური გახადა. გამოტოვებული კვირების ნულით შევსებამ sequence-ში ხელოვნური ვარდნები შექმნა, ამიტომ ექსპერიმენტებით აღმოვაჩინეთ, რომ linear interpolation უკეთეს შედეგს იძლეოდა. Robust scaling-მა კი holiday spikes-ის გავლენა შეამცირა და შედეგი გააუარესა.

ყველაზე საინტერესო ცვლილება არქიტექტურების sweep იყო. Identity stack-ის ამოღებამ და მოდელის Trend + Seasonality კომპონენტებზე ფოკუსირებამ შედეგი გააუმჯობესა. Polynomial degree 3-მა trend-ს მეტი მოქნილობა მისცა, harmonic basis-მა კი recurring seasonal pattern-ების სწავლა გააძლიერა. საბოლოო 1276.74 WMAE მიუთითებს, რომ Walmart-ის გაყიდვებში ისტორიული მიმდევრობა მაინც ძალიან ინფორმაციულია.
...
DLinear-ის ექსპერიმენტებში მთავარი კითხვა იყო, რამდენად შორს შეიძლება წავიდეს ძალიან მარტივი decomposition-based არქიტექტურა. მოდელი moving average-ით გამოყოფს trend-ს და residual/seasonal კომპონენტს, შემდეგ ორივეზე ხაზოვან პროექციას სწავლობს. ამიტომ მისთვის ყველაზე მნიშვნელოვანი ჰიპერპარამეტრი moving-average window აღმოჩნდა.
...
DLinear-მა აჩვენა, რომ სწორად შერჩეული decomposition window-ით მარტივი წრფივი არქიტექტურაც კონკურენტუნარიანია, თუმცა რთული არაწრფივი pattern-ების სწავლაში N-BEATS-ს ჩამორჩა.
...
Validation dataframe არასრული იყო, ამიტომ `make_future_dataframe()`-ით შევქმენით სრული grid და დავამატეთ future covariates. ევალუაცია კი კვლავ მხოლოდ რეალურად არსებულ validation rows-ზე შესრულდა.

### ექსპერიმენტების განვითარება

TFT-ის გამოყენების მთავარი მოტივაცია იყო არა მხოლოდ target history-ის, არამედ წინასწარ ცნობილი future-ების ინფორმაციის გამოყენება. თეორიულად calendar, holiday, MarkDown, store type და macroeconomic features მოდელს საშუალებას აძლევს forecast-ის მიზეზებიც გაითვალისწინოს. პრაქტიკაში კი მდიდარი feature set ყოველთვის უკეთესი არ აღმოჩნდა.

Target-only baseline შედარებით სუსტი იყო. Calendar features-ის დამატებამ შედეგი 1675.41-მდე გააუმჯობესა, რადგან კვირის სეზონურობა და holiday indicator-ები სუფთა და წინასწარ ცნობილი signal-ებია. Static Store Type/Size-ის დამატებამ score გააუარესა; სავარაუდოდ ეს ინფორმაცია Department-level განსხვავებების ასახვისთვის ზედმეტად ზოგადი იყო. MarkDown feature-ებმა sparsity და noise შეიტანეს, ხოლო ყველა exogenous variable-ის ერთდროულმა გამოყენებამ კიდევ უფრო გააუარესა შედეგი.

TFT-ის ექსპერიმენტებმა მოგვცა მნიშვნელოვანი დასკვნების გაკეთების შესაძლებლობა: რთულ multivariate არქიტექტურას არ აქვს აუცილებლად უპირატესობა, თუ დამატებითი feature-ები სუსტი, sparse ან forecast horizon-ისთვის არასაიმედოა. ამ მონაცემებზე მცირე calendar-only კონფიგურაცია ყველაზე სტაბილური იყო.
...
დაახლოებით **3,062 series** წარმატებით დატრეინდა, ხოლო მოკლე/პრობლემური series-ებისთვის გამოიყენებოდა fallback.

### ექსპერიმენტების განვითარება

Prophet-ის ექსპერიმენტები მიზნად ისახავდა trend-ის, yearly seasonality-ისა და holiday effect-ის ცალკე მოდელირებას. განსხვავებით global მოდელებისგან, აქ თითოეული Store–Department series-ისთვის დამოუკიდებელი Prophet მოდელი გამოიყენებოდა. ეს მიდგომა ინტერპრეტირებადია, თუმცა მოკლე და sparse series-ებში ნაკლები ინფორმაციის გაზიარება შეუძლია.

Additive holiday model კარგი საწყისი შედეგი იყო. Holiday component-ის ამოღებამ WMAE გააუარესა, რაც მიუთითებს, რომ დღესასწაულების explicit მოდელირება სასარგებლო აღმოჩნდა. პროგნოზის ისტორიული მაქსიმუმის 1.5-ჯერ მნიშვნელობაზე შეზღუდვამ extreme outputs დაარეგულირა და საუკეთესო 1428.88 WMAE მოგვცა.
...
ARIMA-ს მთავარი შეზღუდვა ის იყო, რომ მას explicit 52-კვირიანი seasonal component არ ჰქონდა. `(2,1,1)` configuration-ში differencing გამოიყენებოდა, თუმცა Walmart-ის სერიებს აშკარად მუდმივი ტრენდი არ ჰქონია და differencing-მა სასარგებლო level ინფორმაცია ნაწილობრივ დაკარგა. `(2,0,1)` კონფიგურაციამ შედეგი ოდნავ გააუმჯობესა, მაგრამ 2139.43 WMAE მხოლოდ მცირედით სჯობდა median baseline-საც კი.

ეს შედეგი გვიჩვენებს, რომ მოკლე autoregressive dependence საკმარისი არ არის ისეთი მონაცემებისთვის, სადაც ერთი წლის წინანდელი იგივე კვირა განსაკუთრებით ინფორმაციულია.
...
TimesFM (Time Series Foundation Model) არის Google Research-ის **pretrained time-series foundation model** — decoder-only transformer (~500M პარამეტრი, ~100 მილიარდ time-point-ზე გაწვრთნილი). განსხვავებით ყველა სხვა მოდელისგან, TimesFM Walmart-ის მონაცემებზე **საერთოდ არ ტრენინგდება**: მუშაობს **zero-shot** რეჟიმში — თითოეული `(Store, Dept)` series-ის ისტორია მიეწოდება როგორც context და მოდელი პირდაპირ აგენერირებს მომდევნო კვირების პროგნოზს.

მოდელი univariate-ია და per-series მუშაობს (როგორც Prophet და ARIMA): იყენებს მხოლოდ target-ის ისტორიას, ყოველგვარი feature engineering-ის გარეშე. სერიები, რომლებსაც საკმარისი ისტორია არ აქვთ, ივსება series-mean → global-mean fallback-ით (99.9% coverage, 42 fallback ~3,062 series-იდან).

გამოყენებულია **TimesFM 2.0 (500M, PyTorch checkpoint)**, alignment ხდება global train cutoff-იდან week-offset-ით, ისე რომ თარიღის string-ის შეუთავსებლობა (W-FRI) პრობლემას არ ქმნის.

### ექსპერიმენტების ინტერპრეტაცია

ყველაზე მნიშვნელოვანი შედეგი: TimesFM-მა zero-shot რეჟიმში, **Walmart-ზე ვარჯიშისა და feature engineering-ის გარეშე**, პრაქტიკულად გაუტოლდა ხელით დატუნინგებულ და ოდნავ აჯობა კიდეც LightGBM-ს (1281 vs 1281). foundation model-ის pretrained ცოდნა საკმარისი აღმოჩნდა ისეთი ხარისხისთვის, რომლის მისაღწევადაც tree მოდელებს ~20 ექსპერიმენტი დასჭირდათ.

**Context length** გადამწყვეტი ფაქტორი აღმოჩნდა. მხოლოდ 52 კვირის (1 წელი) ისტორიით შედეგი მკვეთრად უარესი იყო (1710), რადგან ერთი სეზონური ციკლი საკმარისი არ არის — მოდელს სჭირდება ≥2 სრული წლიური ციკლი წლიური სეზონურობის დასაჭერად. 104 კვირით შედეგი 1309-მდე გაუმჯობესდა, ხოლო სრული ისტორიით (~140 კვირა) — 1281-მდე. დამოკიდებულება მონოტონურია: მეტი context = უკეთესი შედეგი.

**log-space** (`log1p`) პროგნოზი raw-ს პრაქტიკულად გაუტოლდა (1281.07 vs 1281.53), რადგან TimesFM შიგნით უკვე ნორმალიზებს context-ს — გარე log-transform-ს დამატებითი სარგებელი აღარ მოაქვს.

**overfit_gap აქ არ ვრცელდება.** zero-shot მოდელს training fit საერთოდ არ აქვს (Walmart-ზე არ უვარჯიშია), ამიტომ train-vs-val gap-ის ცნება უაზროა — ლოგში პირდაპირ `n/a`-დ არის მონიშნული.

holiday კვირებზე TimesFM უფრო სუსტია (holiday WMAE 1478) ვიდრე non-holiday-ზე (1205) — იგივე pattern, რაც სხვა მოდელებზე, რადგან validation ფანჯარა (ივლ.–ოქტ.) დღესასწაულებს არ შეიცავს.

### ექსპერიმენტები

| ექსპერიმენტი (context / variant) | Validation WMAE | Holiday WMAE | Non-holiday |
|---|---:|---:|---:|
| Context 52 კვირა (1 წელი) | 1710.57 | 1942.59 | 1621.19 |
| Context 104 კვირა | 1309.16 | 1480.42 | 1243.19 |
| Context სრული (~140 კვირა) | 1281.53 | 1459.96 | 1212.79 |
| **log1p (სრული context)** | **1281.07** | 1478.59 | 1204.98 |

Context length-ის ზრდა მონოტონურად აუმჯობესებდა შედეგს; log-space raw-ს გაუტოლდა.

### საუკეთესო შედეგი

| პარამეტრი | მნიშვნელობა |
|---|---|
| მოდელი | TimesFM 2.0 (500M, PyTorch) |
| რეჟიმი | Zero-shot (Walmart-ზე ვარჯიშის გარეშე) |
| Context | სრული series ისტორია (~140 კვირა) |
| Target space | log1p |
| Fallback | series mean → global mean |
| Forecast coverage | 99.9% (42 fallback / 3,062 series) |
| Feature engineering | არავითარი (univariate, მხოლოდ target ისტორია) |
| Validation WMAE | **1281.07** |
| Improvement over baseline | **42.94%** |

TimesFM-ის მთავარი ღირებულება პროექტში — ის არის **მე-4 მოდელის ოჯახი** (pretrained foundation model), რომელმაც training-ისა და feature engineering-ის სრული არარსებობის პირობებში დატუნინგებულ gradient boosting მოდელებს გაუტოლდა. ეს პროექტის ერთ-ერთი ყველაზე საინტერესო შედეგია: ყველაზე დიდი და რთული მოდელი, გამოყენებული ყველაზე ნაკლები ძალისხმევით, კონკურენტული აღმოჩნდა.
---
...
|---:|---|---|---:|---:|
| **1** | **XGBoost** | Global tree-based | **1261.95** | **44.11%** |
| **2** | **N-BEATS** | Global deep learning | **1276.74** | **43.13%** |
| **3** | **TimesFM** | Foundation model (zero-shot) | **1281.07** | **42.94%** |
| 4 | **LightGBM** | Global tree-based | **1281.58** | **42.92%** |
| 5 | DLinear | Global deep learning | 1340.99 | 40.27% |
| 6 | Prophet | Per-series classical | 1428.88 | 36.36% |
| 7 | Seasonal AutoARIMA | Per-series seasonal statistical | 1450.84 | 35.38% |
| 8 | TFT | Global multivariate deep learning | 1675.41 | 25.38% |
| 9 | ARIMA `(2,0,1)` | Per-series statistical | 2139.43 | 4.71% |
...
### 1. ისტორიული გაყიდვები და წლიური სეზონურობა იყო ყველაზე ძლიერი სიგნალი

`lag_52`, `lag_104`, seasonal period 52 და N-BEATS-ის trend/seasonality architecture ყველაზე სტაბილურად გააუმჯობესა შედეგი.
...
### 5. მარტივი მოდელი ზოგჯერ რთულ არქიტექტურასაც ჯობნის

DLinear-მა TFT-ს აჯობა, ხოლო Seasonal Naive/SARIMA TFT-სთან შედარებით კონკურენტული იყო.
